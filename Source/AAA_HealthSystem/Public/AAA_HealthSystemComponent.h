...
public:	
	...
	//Current Health for this blueprint class
	UPROPERTY(Replicated, BlueprintReadWrite, EditAnywhere, Category = "AAA|Health")
	float Health = 100.f;
	//Max Health for this blueprint class(when you add more health , current health can go up to this value)
	UPROPERTY(Replicated, BlueprintReadWrite, EditAnywhere, Category = "AAA|Health")
...
public:	
	...
	//Get Current Health For this Blueprint Class
	UFUNCTION(BlueprintCallable,BlueprintPure, Category = "AAA|Health")
	float GetCurrentHealth();
	//Set Current Health For this Blueprint Class(By Doing this you are Overriding the HealthValue)
	UFUNCTION(BlueprintCallable, Category = "AAA|Health")
	void SetCurrentHealth(float SetHealth);
	//Get Current Health For this Blueprint Class
	UFUNCTION(BlueprintCallable,BlueprintPure, Category = "AAA|Health")
	float GetMaxHealth();
	//Set Max Health For this Blueprint Class(By Doing this you are Overriding the MaxHealthValue)
	UFUNCTION(BlueprintCallable, Category = "AAA|Health")
	void SetMaxHealth(float SetHealth);
	//Add Health in Current Health For this Blueprint Class(Check if you want to exceed the max health value)
	UFUNCTION(BlueprintCallable, Category = "AAA|Health")
	void AddHealth(bool bOverrideMaxHealth,float AddHealth,float& NewHealth);
	//Reduce Health from Current Health For this Blueprint Class(if IsDead means health is Zero or less than zero)
	UFUNCTION(BlueprintCallable, Category = "AAA|Health")
